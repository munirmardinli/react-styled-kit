			...
			<div style={{ display: 'flex', height: '100vh' }}>
				<AppBar position="fixed" open={open}>
					<div style={{ display: 'flex', alignItems: 'center', padding: '0 16px', height: 64 }}>
						<StyledMenuIcon onClick={() => setOpen(!open)} style={{ cursor: 'pointer' }} />
						<div style={{ marginLeft: 16, flexGrow: 1, fontWeight: 'bold', color: 'white' }}>
							My Application
						</div>
				...
				<Drawer variant="permanent" open={open}>
					...
					{navItems.map(({ path, label, icon }) => (
						...
						>
							...
							<StyledListItemText open={open} pathname={pathname} path={path} primary={label} />
						</StyledListItemButton>
					))}
